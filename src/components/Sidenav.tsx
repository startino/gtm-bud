...
export function Sidenav() {
  ...
  return (
    <div className="fixed left-0 top-0 h-full w-[260px] glass border-r border-[var(--border-subtle)] flex flex-col z-10">
      ...
      <div className="flex-1 px-5 py-6 overflow-y-auto">
        <div className="mb-6">
          ...
          <nav className="space-y-1">
            <button
              className="w-full text-left px-3 py-2.5 rounded-md text-sm font-medium bg-[var(--muted)] text-[var(--color-accent)] border-0 transition-premium"
            >
              {campaignName}
            </button>
          </nav>
      ...
      <div className="px-5 py-5 space-y-2 border-t border-[var(--border-subtle)]">
        {/* Theme Toggle */}
        <button
          onClick={toggleTheme}
          className="w-full flex items-center gap-3 px-4 py-2.5 rounded-md hover:bg-transparent transition-premium group border-0 hover:shadow-[var(--shadow-sm)]"
        >
          {theme === 'dark' ? (
        ...
        {/* Profile Button */}
        <button className="w-full flex items-center gap-3 px-4 py-3 rounded-md hover:bg-transparent transition-premium group border-0 hover:shadow-[var(--shadow-sm)]">
